...
LOCAL void bc_comm(void* p)
{
	...
	while(1) {
		...
		aliveTime = (BP_ALIVE_PERIOD - BP_TIMEOUT) * (1000000/BC_COMM_POLL_PERIOD_US);
		...
		while(1) {
			...
			if(0 == ret) {
				...
				tv.tv_sec = 0;
				tv.tv_usec = BC_COMM_POLL_PERIOD_US;
				if(pdPASS == xQueueReceive(g_BcCommQueue, &parseData, 0)) {
					switch(parseData.cmdWord) {
						case CMD_REPORT_SIG_WORD_REQ: 
							{
							BP_SigId2Val sig_val;
							if(parseData.pu.reportSigReq.fieldNum > 0) {
								sig_val.SigId = parseData.pu.reportSigReq.fieldArray[0].signalId;
								sig_val.SigVal.t_u32 = parseData.pu.reportSigReq.fieldArray[0].value.t_u32;

								p_pack_buf = BP_PackReport1SigVal(&BPContextEmbeded, &sig_val);
								if(BP_NULL == p_pack_buf) {
									bc_printf("BP_PackReport1SigVal err\n");
									bc_close(&sockfd_bcserver);
									vTaskDelay(BC_ERROR_DELAY / portTICK_RATE_MS);
									break;
								} 
								n=send(sockfd_bcserver,p_pack_buf->PackStart,p_pack_buf->MsgSize,0);
								if(n != p_pack_buf->MsgSize) {
									bc_printf("* Send reportsig error");
									bc_close(&sockfd_bcserver);
									vTaskDelay(BC_ERROR_DELAY / portTICK_RATE_MS);
									break;
								}
						...
						case CMD_POST_WORD_RSP: 
									      {
							BP_PostackStr postack_str;
							postack_str.RetCode = 0;
							p_pack_buf = BP_PackPostack(&BPContextEmbeded, &postack_str);
							if(BP_NULL == p_pack_buf) {
								bc_printf("BP_PackPostack err\n");
								bc_close(&sockfd_bcserver);
								vTaskDelay(BC_ERROR_DELAY / portTICK_RATE_MS);
								break;
							} 
			...
			if(FD_ISSET(sockfd_bcserver, &rfds)) {
				...
				if(((type_and_flags >> 4) & 0xF) == BP_PACK_TYPE_POST) {
					...
					for(i = 0; i < str_post.SigNum; i++) {
						...
						if(tmp->SigId == SIG_SYS_POWER) {
							parseData.cmdWord = CMD_POST_WORD_REQ;
							parseData.pu.postReq.fieldArray = &postField;
							parseData.pu.postReq.fieldNum = 1;
							postField.signalId = SIG_SYS_POWER;
							postField.signalType = BPS_SIG_TYPE_BOOLEAN;
							if(tmp->SigVal.t_u16 > 0) {
								postField.value.t_bool = 1;
							} else {
								postField.value.t_bool = 0;
							}
							xQueueSendToBack(g_ComQueue, &parseData, 0);
						}
					}
					bc_printf("post ok\n");
					...
					bc_printf("* PINGACK:\n");
				} else if(((type_and_flags >> 4) & 0xF) == BP_PACK_TYPE_RPRTACK) {
					/* parse BP Packet RPRTACK*/
					bc_printf("* RPRTACK:\n");
					BP_RprtackStr str_rprtack;
					if(0 != BP_ParseRprtack(&BPContextEmbeded, &str_rprtack, RECV_BUF, len)) {
						bc_printf("* parse rprtack err:\n");
						bc_close(&sockfd_bcserver);
						vTaskDelay(BC_ERROR_DELAY / portTICK_RATE_MS);
						break;
					}
					parseData.cmdWord = CMD_REPORT_SIG_WORD_RSP;
					if(str_rprtack.RetCode == RET_CODE_RPRTACK_OK) {
						parseData.pu.postRsp.retCode = BPS_RET_CODE_OK;
					} else {
						parseData.pu.postRsp.retCode = BPS_RET_CODE_FAILED;
					}
					xQueueSendToBack(g_ComQueue, &parseData, 0);
				} else {
